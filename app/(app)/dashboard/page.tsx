      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-gray-900">Upcoming Contributions</h2>
            <Link href="/groups" className="text-sm text-teal-600 hover:underline flex items-center gap-1">
              View all <ArrowRight size={14} />
            </Link>
          </div>
          {upcoming.length === 0 ? (
            <p className="text-gray-500 text-sm py-6 text-center">No upcoming contributions</p>
          ) : (
            <div className="space-y-3">
              {upcoming.slice(0, 5).map(c => (
                <div key={c.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div>
                    <p className="text-sm font-medium text-gray-900">{c.groupName || 'Group'}</p>
        ...
        <Card>
          ...
          {notifs.length === 0 ? (
            <p className="text-gray-500 text-sm py-6 text-center">No new notifications</p>
          ) : (
            <div className="space-y-3">
              {notifs.map(n => (
                <div key={n.id} className={`p-3 rounded-lg ${n.isRead ? 'bg-gray-50' : 'bg-teal-50 border-l-2 border-teal-500'}`}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {myGroups.slice(0, 6).map(g => (
              <Link key={g.id} href={`/groups/${g.id}`}>
                <div className="border border-gray-100 rounded-xl p-4 hover:border-teal-200 hover:shadow-sm transition-all cursor-pointer">
